...
fn manual_install_cmd(_channel: &str) -> &'static str {
...
pub async fn run_install_script(
    ...
    let result: Result<Option<String>> = match installer {
        ...
        "gh-release" | "internal" => install_gh_release(target).await.map(|()| None),
        _ => install_gh_release(target).await.map(|()| None),
    };
...
fn warn_if_other_grok_processes_running() {
    ...
    // Match this product (`grok-cli`), not every process whose cmdline contains
    // the substring "grok" (models, official CLI, unrelated tools).
    cmd.args(["-f", "grok-cli"])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    ...
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other grok-cli process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other grok-cli sessions before updating.");
            eprintln!();
        }
    }
}
...
pub async fn run_update(
    ...
    {
        eprintln!(
            "Forcing reinstall of grok-cli {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
