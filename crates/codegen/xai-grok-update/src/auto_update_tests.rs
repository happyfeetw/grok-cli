...
fn test_needs_update_stable_accepts_numeric_fork_prerelease() {
    ...
    // Named pre-releases are still rejected.
    assert_eq!(
...
fn test_reinstall_hint_gh_release_mentions_gh_command() {
    ...
    assert!(
        ...
        "should name the fork release page: {hint}"
    );
    assert!(
        hint.contains("@spikewang/grok-cli"),
        "should retain the fork npm fallback: {hint}"
    );
}

#[test]
fn test_reinstall_hint_internal_mentions_fork_release() {
    let hint = reinstall_hint("internal", "stable");
    assert!(
        hint.contains("happyfeetw/grok-cli"),
        "internal must route to fork releases: {hint}"
    );
    assert!(
        !hint.contains("x.ai/cli"),
        "fork must not suggest the official installer: {hint}"
    );
}

#[test]
fn test_reinstall_hint_unknown_falls_back_to_manual() {
    for channel in ["stable", "alpha", "enterprise", "al pha", ""] {
        let hint = reinstall_hint("homebrew", channel);
        assert!(
            hint.contains("@spikewang/grok-cli"),
            "unknown installer must suggest fork npm install: {hint}"
        );
    }
}
...
fn test_reinstall_hint_empty_falls_back_to_manual() {
    ...
    assert!(
        hint.contains("@spikewang/grok-cli"),
        "empty installer must suggest fork npm install: {hint}"
    );
}
