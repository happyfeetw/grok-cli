...
pub fn run(shell: Shell) {
    // Product command for this fork is `grok-cli` (not official `grok`).
    let mut cmd = PagerArgs::command().name("grok-cli");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "grok-cli", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "grok-cli", &mut buf);
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        // Product name is `grok-cli` (clap bin name).
        (
            r#"curcontext="${curcontext%:*:*}:grok-cli-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:grok-cli-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("grok-cli");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "grok-cli", &mut buf);
    ...
    fn zsh_completions_drop_prompt_slot_and_dispatch_on_line_1() {
        ...
        assert!(
            fixed.contains(r#"curcontext="${curcontext%:*:*}:grok-cli-command-$line[1]:""#),
            "root dispatch context must use $line[1]"
        );
        // Subcommand dispatch blocks (already on $line[1]) must survive.
        assert!(
            fixed.contains("grok-cli-worktree-command-$line[1]")
        ...
        assert!(
            fixed.contains("_grok_cli_commands") || fixed.contains("_grok_commands"),
