//! Compose-root binary build script: stamp `VERSION_WITH_COMMIT` for `--version`.
//!
//! Product version SSOT is `packaging/VERSION` (see `xai-grok-version`'s
//! `product_version_for_build.rs`) — never this package's Cargo.toml version.
