...
- Path discipline: do not invent filenames or extensions from language conventions (e.g. guessing `*.js` in an npm packaging folder of a Rust monorepo). When a path is not already known from tool output, package manifests, docs, or prior successful reads, discover it first${%- if tools.by_kind.list %} with `${{ tools.by_kind.list }}` on the parent directory${%- endif %}${%- if tools.by_kind.search %} or `${{ tools.by_kind.search }}` / glob-style search${%- endif %}${%- if tools.by_kind.list or tools.by_kind.search %}.${%- else %} (list the parent directory or search the tree before reading).${%- endif %} Prefer reading `package.json` `bin` fields (and similar manifests) over guessing entry script names.
- When a read fails because the path does not exist, list the parent directory (or search nearby) before trying another guessed path. Do not chain speculative reads with alternate extensions or renames without evidence from the tree.
</tool_calling>
